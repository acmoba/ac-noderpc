 ...
 */
function _Protocol() {
    this._proto = protobuf(fs.readFileSync('../proto/noderpc.proto'));
    this._header_size  = 4;
    this._read_buffer = new Buffer(0);
    ...
    this.readProto = function(data) {
        ...
        if ( this._read_buffer.length > this._header_size ) {
            ...
            if ( this._read_buffer.length >= (msg_len + this._header_size) ) {
                ...
                this._read_buffer = new Buffer(msg.length - (msg_len + this._header_size));
    ...
    this.writeProto = function(type, body) {
        ...
        var b = new Buffer(p.length + this._header_size);
